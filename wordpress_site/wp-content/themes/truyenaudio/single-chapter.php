...
jQuery(function($) {
    ...
    if ($readerContent.length) {
        ...
        $('#settings-more-btn').on('click', function(e){
            ...
            if (ttsPlaying) {
                $('#tts-btn-exp').addClass('active').text(ttsPaused ? '▶️' : '⏹');
            }
        });
        $(document).on('click', function(){ $('#settings-expanded').removeClass('show'); });

        // ========== Vietnamese TTS (Web Speech API) ==========
        var ttsChunkSize = 1500;
        var ttsChunks = [];
        var ttsIndex = 0;
        ...
        function getVietVoice() {
            var voices = synth.getVoices();
            var vi = voices.filter(function(v){ return v.lang.indexOf('vi') === 0; });
        ...
        function chunkText(text) {
            ...
            for (var i = 0; i < sentences.length; i++) {
                if ((current + ' ' + sentences[i]).length > ttsChunkSize && current.length > 0) {
        ...
        function ttsStart() {
            ...
            ttsPlaying = true;
            ttsPaused = false;
            $('#tts-btn, #tts-btn-exp').addClass('active').text('⏹');
            ta_toast('Bắt đầu đọc truyện...', 'success');
            ttsSpeakChunk();
        }
        ...
        $(window).on('beforeunload', function() { if (ttsPlaying) synth.cancel(); });

        // Load voices (Chrome loads async)
        if (synth.onvoiceschanged !== undefined) {
            synth.onvoiceschanged = function() { getVietVoice(); };
        }
    }
});
</script>
